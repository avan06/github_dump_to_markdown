...
Options:
  ...
  --url            GitHub Repository URL, simplifies input by determining owner, repo, dumptype, and numbers
...
async def main():
    ...
    def parse_github_url(url):
        ...
        # Determine the type and number if applicable
        dumptype = None
        number = None
        if len(path_parts) > 2:
            ...
            # Extract the number if the type is valid and a number exists
            if dumptype and len(path_parts) > 3 and path_parts[3].isdigit():
                number = int(path_parts[3])
    
        return owner, repo, dumptype, number
    ...
    parser.add_argument("--url", help="GitHub Repository URL, simplifies input by determining owner, repo, dumptype, and numbers")
    ...
    if args.url:
        args.owner, args.repo, dumptype, number = parse_github_url(args.url)
        if dumptype:
